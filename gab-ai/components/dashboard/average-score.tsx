...
export const AverageScoreCard: React.FC = () => {
  ...
  React.useEffect(() => {
    async function fetchAverageScore() {
      try {
        const response = await fetch('/api/average-score');
        const data = await response.json();
        if (response.ok) {
          setAverageScore(data.averageScore);
        } else {
          console.error('Failed to fetch average score:', data.error);
        }
      } catch (error) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-[0_2px_4px_rgba(0,0,0,0.02)] flex flex-col justify-between">
      ...
      <div className="mt-6">
        <div className="flex items-baseline gap-1 mb-4">
          <span className="text-5xl font-semibold tracking-tighter text-gray-900">{averageScore !== null ? averageScore : '--'}</span>
          <span className="text-xl text-gray-400 font-light">/100</span>
        </div>
        <div className="w-full bg-gray-100 rounded-full h-2 mb-2 overflow-hidden">
          <div className="bg-gray-900 h-2 rounded-full" style={{ width: averageScore !== null ? `${averageScore}%` : '0%' }}></div>
        </div>
